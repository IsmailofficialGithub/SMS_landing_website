import React, { useState, useEffect } from 'react';
import { Link, useLocation } from 'react-router-dom';
import { Phone, Menu, X, ArrowRight, Sparkles } from 'lucide-react';
import './Navbar.css';

const Navbar = ({ openQuote }) => {
  const [isOpen, setIsOpen] = useState(false);
  const location = useLocation();

  // Close mobile menu whenever route changes
  useEffect(() => {
    setIsOpen(false);
  }, [location.pathname]);

  // Prevent background scroll when mobile menu is open
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = 'hidden';
    } else {
      document.body.style.overflow = '';
    }
    return () => {
      document.body.style.overflow = '';
    };
  }, [isOpen]);

  const navLinks = [
    { to: '/', label: 'Home' },
    { to: '/products', label: 'Products' },
    { to: '/gallery', label: 'Gallery' },
    { to: '/projects', label: 'Projects' },
    { to: '/blog', label: 'Blog' },
    { to: '/about', label: 'About Us' },
    { to: '/contact', label: 'Contact Us' },
  ];

  return (
    <header className="header">
      <div className="top-bar">
        <div className="container top-bar-container">
          <div className="top-bar-left">
            <a href="tel:+923334727284" aria-label="Call Spectrum Digital">
              <Phone size={13} />
              <span>+92 333 4727284</span>
            </a>
          </div>
          <div className="top-bar-center">
            <div className="marquee">
              <span>Distributor of Samsung, Philips, LianTronics, & Unilumin A/V Solutions in Pakistan.</span>
            </div>
          </div>
          <div className="top-bar-right">
            <a href="#" aria-label="Facebook">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path></svg>
            </a>
            <a href="#" aria-label="Instagram">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line></svg>
            </a>
            <a href="#" aria-label="Twitter">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"></path></svg>
            </a>
            <a href="#" aria-label="Youtube">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"></path><polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"></polygon></svg>
            </a>
          </div>
        </div>
      </div>

      <nav className="main-navbar">
        <div className="container nav-container">
          <div className="nav-logo">
            <Link to="/" onClick={() => setIsOpen(false)}>
              <img src="/logo.png" alt="Spectrum Digital Logo" />
            </Link>
          </div>

          <ul className="nav-links">
            {navLinks.map((item) => (
              <li key={item.to}>
                <Link
                  to={item.to}
                  className={location.pathname === item.to ? 'active' : ''}
                >
                  {item.label}
                </Link>
              </li>
            ))}
          </ul>

          <div className="nav-actions">
            {openQuote && (
              <button className="nav-quote-btn" onClick={openQuote}>
                <Sparkles size={14} /> Get Quote
              </button>
            )}
            <button
              className="mobile-toggle-btn"
              onClick={() => setIsOpen(!isOpen)}
              aria-label={isOpen ? 'Close menu' : 'Open menu'}
              aria-expanded={isOpen}
            >
              {isOpen ? <X size={24} /> : <Menu size={24} />}
            </button>
          </div>
        </div>

        <div className={`mobile-menu-drawer ${isOpen ? 'open' : ''}`}>
          <div className="mobile-menu-inner">
            <ul className="mobile-nav-list">
              {navLinks.map((item) => (
                <li key={item.to} className="mobile-nav-item">
                  <Link
                    to={item.to}
                    className={location.pathname === item.to ? 'active' : ''}
                    onClick={() => setIsOpen(false)}
                  >
                    <span>{item.label}</span>
                    <ArrowRight size={16} />
                  </Link>
                </li>
              ))}
            </ul>

            <div className="mobile-menu-actions">
              {openQuote && (
                <button
                  className="mobile-action-quote"
                  onClick={() => {
                    setIsOpen(false);
                    openQuote();
                  }}
                >
                  <Sparkles size={16} /> Get Free Quote
                </button>
              )}
              <a
                href="tel:+923334727284"
                className="mobile-action-call"
                onClick={() => setIsOpen(false)}
              >
                <Phone size={15} /> Call: +92 333 4727284
              </a>
            </div>
          </div>
        </div>
      </nav>
    </header>
  );
};

export default Navbar;
